...
DepositAccount::DepositAccount(uint64_t id, double balance, time_t time)
    : Account(id, balance), time_(time) {}

void DepositAccount::Withdraw(double amount) {
  if (std::time(nullptr) < time_) {
    std::cout << "Снятия недоступны до " << time_ << '\n';
  } else {
...
void DepositAccount::Deposit(double amount) {
  if (std::time(nullptr) < time_) {
    std::cout << "Пополнение недоступно до " << time_ << '\n';
  } else {
    Account::balance_ += amount;
  }
}
